...
export const fetchTopSellingProductsByStoreId = createAsyncThunk(
  ...
  async (storeId: string, thunkAPI) => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/stores/${storeId}/top-selling-products`);
      return response.data as FetchProductsPayload;
    } catch (error) {
      if (axios.isAxiosError(error) && error.response) {
        return thunkAPI.rejectWithValue(error.response.data.message);
...
const productSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchTopSellingProductsByStoreId.fulfilled, (state, action: PayloadAction<FetchProductsPayload>) => {
      state.loading = false;
      state.topSellingProducts = action.payload.results;
    });
    builder.addCase(fetchTopSellingProductsByStoreId.rejected, (state, action) => {
      state.loading = false;
      state.error = action.payload as string;
    });
...
export const fetchStoreById = createAsyncThunk('stores/fetchStoreById', async (storeId: string, thunkAPI) => {
  try {
    ...
    if (axios.isAxiosError(error) && error.response) {
      console.error(`Error fetching store by ID: ${error.response.status} - ${error.response.data.message}`);
      return thunkAPI.rejectWithValue(error.response.data.message);
    } else {
